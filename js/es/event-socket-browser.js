...
(function() {
  ...
  var streamFile = function(self, file, data, conn) {
    var delay = data.delay || 0*1000; // delay between chunk reads
    ...
    var sendChunk = function(evt) {
      if(evt.target.error == null) {
        ...
        conn.send(payload);
        $(file).trigger('progress', { bytes: offset, total: fileSize, percent: Math.floor(offset/fileSize*100), status: "info" });
        count += 1;
      } else {
        console.error("Error reading file ", evt.target.error);
        var pct = Math.floor(offset/fileSize*100);
        $(file).trigger('progress', { bytes: offset, total: fileSize, percent: pct, status: "danger" });
        return;
      }
      ...
      if(offset >= fileSize) {
        ...
        conn.send(payload);
        $(file).trigger('progress', { bytes: offset, total: fileSize, percent: 100, status: "success" });
        return;
      }
      setTimeout(function() {
